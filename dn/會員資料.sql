select posc.code "發卡店代", posc.name "發卡店櫃", m.code "會員代號", m.name "會員名稱", m.membercardno "卡號",
       m.firstdate "發卡日期",left(m.firstdate,4) "發卡年",substring(m.firstdate,5,2) "發卡月",substring(m.firstdate,7,2) "發卡日",
       m.Birthday "生日",year(getdate())-left(m.Birthday,4) "年齡" , m.HomeTel "市話" ,m.CellPhone "手機" ,m.HomeAddress "地址",m.OfficeAddress "公司地址",
